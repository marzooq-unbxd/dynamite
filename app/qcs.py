...
ASTERIX_API = "{}/v2/cores/{}/analyzers/search?format=raw"   #analysed query but no distinction betweeen mts etc
...
def clean_textv2( text, isite_name):
    try:
        ...
        url = ASTERIX_API.format(config.ASTERIX_URL, isite_name)
        # response = requests.get(url,params={'q': requests.utils.quote(text)})
...
def add_dynamic_mts_to_analyzed_query(analyzed_query, query_dynamic_mts):
    ...
    analyzed_query=analyzed_query['analyzed_query']
    analyzed_query_tkns = analyzed_query.split()

    for i in query_dynamic_mts:
        pos_dynamic_mt = [x for x in analyzed_query_tkns if i ==x or x.startswith(str(i)+'<')]
        if len(pos_dynamic_mt)>1:
            continue
        else:
            analyzed_query_tkns = ['+' + x if x==i else x for x in analyzed_query_tkns]

    return {'analyzed_query':" ".join(analyzed_query_tkns)}
    ...
    for tkn in query_tkns:
        # if tkn not in original_cleaned_hashmap:
            if tkn not in act_mt:
                if not (any(x.isdigit() for x in tkn))  :  #another hadk 20mm has a concept, hence avoiding any numbered token
...
def get_statistical_categoryv2(site):
    ...
    try:
        ...
        logger.error(
            "Error while fetching analyzed query for site: {}, query: {}, error: {}".format (site, query, str(e)))
